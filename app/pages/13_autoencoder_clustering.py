...
from src.autoencoder_clustering import (
    run_autoencoder_kmeans,
    evaluate_autoencoder_kmeans_range,
)

st.set_page_config(page_title="Autoencoder + KMeans", layout="wide")
init_session_state()
...
st.title("Autoencoder + KMeans")
st.write("Нейросетевой метод: автоэнкодер для сжатия признаков и KMeans в латентном пространстве.")
...
st.subheader("Параметры автоэнкодера")
...
with col_k1:
    default_k_min = st.session_state.get("ae_k_min", 2)
...
with col_k2:
    default_k_max = st.session_state.get("ae_k_max", 6)
...
if k_min >= k_max:
    ...
    if st.button("Оценить k", key="evaluate_ae_button"):
        ...
        if len(current_features) < 3:
            ...
            if min_allowed_k > max_allowed_k:
                ...
                st.session_state["ae_k_scores_df"] = evaluate_autoencoder_kmeans_range(
                    current_features,
...
if st.session_state["ae_k_scores_df"] is not None:
    st.subheader("Метрики для разных k")
    st.dataframe(st.session_state["ae_k_scores_df"], use_container_width=True)
...
default_clusters = st.session_state.get("ae_n_clusters", 4)
...
if st.button("Запустить Autoencoder + KMeans", key="run_ae_button"):
    current_features = st.session_state["features_df_for_clustering"]

    if len(current_features) < n_clusters:
        st.error("Число пользователей меньше числа кластеров. Уменьшите k.")
    else:
        st.session_state["ae_clustering_result"] = run_autoencoder_kmeans(
            current_features,
            latent_dim=latent_dim,
...
if st.session_state["ae_clustering_result"] is not None:
    clustering_result = st.session_state["ae_clustering_result"]

    result_df = clustering_result["result_df"]
    latent_df = clustering_result["latent_df"]
    ...
    m4.metric("Train loss", f"{metrics['final_train_loss']:.6f}")
    m5.metric(
        "Val loss",
        "-" if metrics["final_val_loss"] is None else f"{metrics['final_val_loss']:.6f}"
    )

    st.subheader("Латентное представление")
    st.dataframe(latent_df, use_container_width=True)
    ...
    selected_feature = st.selectbox(
        ...
        available_features,
        key="selected_ae_cluster_feature"
    )
    ...
    default_student = st.session_state.get("selected_ae_student_id")
    ...
    show_only_strong = st.checkbox(
        ...
        value=False,
        key="show_only_strong_ae_deviation"
    )
    ...
    st.info("Выберите параметры и нажмите «Запустить Autoencoder + KMeans».")