...
st.write(
    "На этой странице можно выбрать методы для сравнения, сопоставить их метрики, "
    "посмотреть совпадение поведенческих паттернов у студентов и сравнить переходы "
    "между двумя методами с помощью Sankey-диаграммы."
)
...
# ------------------------------------------------------------
st.subheader("Автоматический вывод по сравнению методов")
...
# ------------------------------------------------------------
st.subheader("Рекомендация для итоговой ВКР")

st.info(
    "Для итоговых выводов лучше сравнивать методы не по номерам кластеров, "
    "а по названиям поведенческих паттернов и средним профилям. "
    "KMeans можно оставить основным методом благодаря простоте интерпретации, "
    "Agglomerative использовать для проверки устойчивости, GMM — для анализа "
    "смешанных профилей, HDBSCAN — для поиска нетипичных студентов, а DEC — "
    "как дополнительный нейросетевой эксперимент."
)