...
st.set_page_config(page_title="PM Agglomerative Clustering", layout="wide")
init_session_state()
...
st.title("PM Agglomerative Clustering")
st.write("Иерархическая кластеризация по process mining-признакам.")
...
with col_k1:
    default_k_min = st.session_state.get("pm_agg_k_min", 2)
...
with col_k2:
    default_k_max = st.session_state.get("pm_agg_k_max", 6)
...
if k_min >= k_max:
    ...
    if st.button("Оценить k", key="evaluate_pm_agg_button"):
        ...
        if len(current_features) < 3:
            ...
            if min_allowed_k > max_allowed_k:
                st.error("Слишком мало данных для выбранного диапазона k.")
            else:
                st.session_state["pm_agg_k_scores_df"] = evaluate_agglomerative_range(
                    current_features,
                    k_min=min_allowed_k,
                    k_max=max_allowed_k,
                    linkage=linkage,
...
if st.session_state["pm_agg_k_scores_df"] is not None:
    st.subheader("Метрики для разных k")
    st.dataframe(st.session_state["pm_agg_k_scores_df"], use_container_width=True)
...
default_clusters = st.session_state.get("pm_agg_n_clusters", 4)
...
n_clusters = st.slider(
    ...
    step=1,
    key="pm_agg_n_clusters"
)

if st.button("Запустить PM Agglomerative Clustering", key="run_pm_agg_button"):
    current_features = st.session_state["pm_features_df"]

    if len(current_features) < n_clusters:
        st.error("Число пользователей меньше числа кластеров. Уменьшите k.")
    else:
        st.session_state["pm_agg_clustering_result"] = run_agglomerative(
            current_features,
...
# Результаты
# -----------------------------
if st.session_state["pm_agg_clustering_result"] is not None:
    clustering_result = st.session_state["pm_agg_clustering_result"]
    ...
    selected_feature = st.selectbox(
        ...
        available_features,
        key="selected_pm_agg_cluster_feature"
    )
    ...
    default_student = st.session_state.get("selected_pm_agg_student_id")
    ...
    selected_student_id = st.selectbox(
        ...
        key="selected_pm_agg_student_id"
    )
    ...
    show_only_strong = st.checkbox(
        ...
        value=False,
        key="show_only_strong_pm_agg_deviation"
    )
    ...
    st.info("Выберите параметры и нажмите «Запустить PM Agglomerative Clustering».")