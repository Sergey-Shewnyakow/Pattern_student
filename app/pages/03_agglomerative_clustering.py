...
from src.agglomerative_clustering import (
    ...
    compare_agglomerative_linkages,
)
...
st.set_page_config(
    page_title="Agglomerative Clustering",
    layout="wide",
)
...
st.write(
    "Иерархическая кластеризация подготовленных данных методом "
    "Agglomerative Clustering. Метод использует тот же набор признаков, "
    "что и KMeans, чтобы результаты можно было сравнивать между собой."
)
...
with col_k1:
    default_k_min = st.session_state.get("agglomerative_k_min", 2)
    ...
    k_min = st.number_input(
        ...
        value=default_k_min,
        step=1,
        key="agglomerative_k_min",
    )

with col_k2:
    default_k_max = st.session_state.get("agglomerative_k_max", 6)
    ...
    k_max = st.number_input(
        ...
        value=default_k_max,
        step=1,
        key="agglomerative_k_max",
    )

with col_k3:
    linkage_for_range = st.selectbox(
        "Linkage для оценки k",
        options=["ward", "average", "complete"],
        index=0,
        key="agglomerative_linkage_for_range",
    )
...
if k_min >= k_max:
    ...
    if st.button(
        "Оценить k",
        key="evaluate_agglomerative_button",
    ):
        ...
        if len(current_features) < 3:
            ...
            if min_allowed_k > max_allowed_k:
                ...
                st.session_state["agglomerative_scores_df"] = (
                    evaluate_agglomerative_range(
                        features_df=current_features,
                        k_min=min_allowed_k,
                        k_max=max_allowed_k,
                        linkage=linkage_for_range,
                    )
                )

if st.session_state.get("agglomerative_scores_df") is not None:
...
# Сравнение linkage
# ------------------------------------------------------------
st.subheader("Сравнение вариантов linkage")

st.caption(
    "Ward обычно используется как основной вариант для числовых стандартизированных "
    "признаков. Average и Complete можно использовать как проверочные варианты."
)

if st.button(
    "Сравнить linkage",
    key="compare_agglomerative_linkages_button",
):
    default_clusters_for_linkage = st.session_state.get(
        "agglomerative_n_clusters",
        4,
    )

    st.session_state["agglomerative_linkage_comparison_df"] = (
        compare_agglomerative_linkages(
            features_df=features_df_for_clustering,
            n_clusters=default_clusters_for_linkage,
        )
    )

if st.session_state.get("agglomerative_linkage_comparison_df") is not None:
    st.dataframe(
        st.session_state["agglomerative_linkage_comparison_df"],
        use_container_width=True,
    )
...
if max_possible_k < 2:
    st.error("Недостаточно данных для Agglomerative Clustering.")
    st.stop()

default_clusters = st.session_state.get("agglomerative_n_clusters", 4)
...
with col_p2:
    linkage = st.selectbox(
        ...
        options=["ward", "average", "complete"],
        index=0,
        key="agglomerative_linkage",
    )

st.caption(
    "Для основного сравнения с KMeans рекомендуется использовать 4 кластера "
    "и linkage='ward'."
)
...
# ------------------------------------------------------------
if st.button(
    "Запустить Agglomerative Clustering",
    key="run_agglomerative_button",
):
    current_features = st.session_state["features_df_for_clustering"]
    ...
    if len(current_features) < n_clusters:
        ...
        st.session_state["agglomerative_result"] = run_agglomerative(
            features_df=current_features,
...
# Результаты
# ------------------------------------------------------------
if st.session_state.get("agglomerative_result") is not None:
    agglomerative_result = st.session_state["agglomerative_result"]

    result_df = agglomerative_result["result_df"]
    metrics = agglomerative_result["metrics"]
    cluster_profiles = agglomerative_result["cluster_profiles"]
    used_features = agglomerative_result["used_features"]
    ...
    cluster_names_df = render_editable_cluster_names(
        ...
        title="Названия кластеров Agglomerative Clustering",
    )
    ...
    if available_features:
        default_feature = st.session_state.get(
            "selected_agglomerative_cluster_feature",
        ...
        selected_feature = st.selectbox(
            ...
            key="selected_agglomerative_cluster_feature",
        )
    ...
    if student_ids:
        default_student = st.session_state.get("selected_agglomerative_student_id")
        ...
        selected_student_id = st.selectbox(
            ...
            key="selected_agglomerative_student_id",
        )
        ...
        show_only_strong = st.checkbox(
            ...
            value=False,
            key="show_only_strong_deviation_agglomerative",
        )
    ...
    st.info("Выберите параметры и нажмите «Запустить Agglomerative Clustering».")