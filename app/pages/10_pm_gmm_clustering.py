...
st.set_page_config(page_title="PM Gaussian Mixture Models", layout="wide")
init_session_state()
...
st.title("PM Gaussian Mixture Models")
st.write("Кластеризация process mining-признаков методом Gaussian Mixture Model.")
...
with col_k1:
    default_k_min = st.session_state.get("pm_gmm_k_min", 2)
...
with col_k2:
    default_k_max = st.session_state.get("pm_gmm_k_max", 6)
...
if k_min >= k_max:
    ...
    if st.button("Оценить k", key="evaluate_pm_gmm_button"):
        ...
        if len(current_features) < 3:
            ...
            if min_allowed_k > max_allowed_k:
                ...
                st.session_state["pm_gmm_k_scores_df"] = evaluate_gmm_range(
                    current_features,
                    k_min=min_allowed_k,
                    k_max=max_allowed_k,
...
if st.session_state["pm_gmm_k_scores_df"] is not None:
    st.subheader("Метрики для разных k")
    st.dataframe(st.session_state["pm_gmm_k_scores_df"], use_container_width=True)
...
default_clusters = st.session_state.get("pm_gmm_n_clusters", 4)
...
n_clusters = st.slider(
    ...
    step=1,
    key="pm_gmm_n_clusters"
)

if st.button("Запустить PM GMM", key="run_pm_gmm_button"):
    current_features = st.session_state["pm_features_df"]

    if len(current_features) < n_clusters:
        st.error("Число пользователей меньше числа компонент. Уменьшите k.")
    else:
        st.session_state["pm_gmm_clustering_result"] = run_gmm(
            current_features,
...
# Результаты
# -----------------------------
if st.session_state["pm_gmm_clustering_result"] is not None:
    clustering_result = st.session_state["pm_gmm_clustering_result"]
    ...
    selected_feature = st.selectbox(
        ...
        available_features,
        key="selected_pm_gmm_cluster_feature"
    )
    ...
    default_student = st.session_state.get("selected_pm_gmm_student_id")
    ...
    selected_student_id = st.selectbox(
        ...
        key="selected_pm_gmm_student_id"
    )
    ...
    show_only_strong = st.checkbox(
        ...
        value=False,
        key="show_only_strong_pm_gmm_deviation"
    )
    ...
    st.info("Выберите параметры и нажмите «Запустить PM GMM».")